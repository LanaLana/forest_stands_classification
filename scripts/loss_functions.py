...
def per_stand_loss(alpha = 1,beta = 1):
    def loss(y_true,y_pred):
        ...
        prob = tf.reduce_sum(tf.where(tf.equal(tf.clip_by_value(y_pred*2,_epsilon,1), tf.ones_like(y_pred)), 
                                  tf.ones_like(y_pred)*y_pred, 
                                  tf.zeros_like(y_pred)),[1, 2]) / n_pix
        ...
        return -K.mean(result, axis=-1)*10#-K.mean(result) * 10
